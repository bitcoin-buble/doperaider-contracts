...
contract NarcoMinting is NarcoAuction {
    ...
    uint256 public gen0StartingPrice = 2000*1 ether;
    uint256 public gen0EndingPrice = 2000*20 finney;
...
contract DopeRaiderCore is NarcoMinting {
   ...
   function getNarco(uint256 _id)
    ...
    // this function to be called by bridge to migrate from chain to chain
    // or to be called in the event of some corruption
    function recoverNarco
    (
     uint256 _id,
     string  _narcoName,
     uint16 _weedTotal,
     ...
     address _recoveryAddress
         ...
         require(_id<=narcos.length);

         if(_id==narcos.length){
           // create a new narco
           uint256[6] memory cools;

           Narco memory _narco = Narco({
               genes: _genes,
               narcoName: _narcoName,
               cooldowns: cools,
               stats: _stats,
               weedTotal: _weedTotal,
               cokeTotal: _cokeTotal,
           ...
           narcos.push(_narco);
           NarcoCreated(_recoveryAddress, _id, narcos[_id].genes);
         }else{
           // recover an existing narco

           narcos[_id].narcoName = _narcoName;
           narcos[_id].weedTotal = _weedTotal;
           narcos[_id].cokeTotal = _cokeTotal;
           narcos[_id].skills = _skills;
           narcos[_id].consumables=_consumables;
           narcos[_id].genes = _genes;
           narcos[_id].homeLocation = _homeLocation;
           narcos[_id].stats = _stats;
         }

         _transfer(0, _recoveryAddress, _id);
     }
    ...
    uint256 public personalisationCost = 2000*0.01 ether; // pimp my narco
   ...
   // crafting section
    uint256 public growCost = 2000*0.006 ether;
    ...
    uint256 public refineCost = 2000*0.006 ether;
