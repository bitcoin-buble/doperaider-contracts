...
// DopeRaider Districts Contract V4
// by gasmasters.io
...
contract NarcosCoreInterface is ERC721 {
  ...
  function getLevel(uint256 _narcoId) public view returns (uint16 level);
}


contract CalculationsInterface {
  function isBusted(uint16 _speedSkill) public returns (bool busted);
  function isRaided(uint16 _respectAttacker, uint16 _respectVictim, uint16 _attackSkill , uint16 _defenseSkill) public view returns (bool raided);
}
...
contract DistrictsAdmin is Ownable, Pausable {
  ...
  CalculationsInterface calculationsContract;

  function seCalculationsContractAddress(address _address) public onlyOwner {
    calculationsContract = CalculationsInterface(_address);
  }
...
contract DistrictsCore is DistrictsAdmin {
  ...
  event SaleComplete(uint256 indexed narcoId, uint256 indexed amount);

  uint256 public airLiftPrice = 40 ether; // home dorothy prices in POA
  uint256 public hijackPrice = 10 ether; // universal hijackPrice in POA
  uint256 public travelPrice = 2 ether; // universal travelPrice in POA
  uint256 public spreadPercent = 10; // universal spread between buy and sell
  uint256 public devFeePercent = 2; // on various actions
  uint256 public currentDevFees = 0;
  uint256 public bustRange = 10;
  bool public homeDistrictRaiding = true;
  uint256 public localRevenuePercent = 25;

  uint256 public minBuyQtyWeed = 12;
  uint256 public minBuyQtyCoke = 4;

  uint256 public endCoolDownPrice = 15 ether;
  uint256 public saleDangerZone = 30;
  ...
  function setMinBuyQtyWeed(uint256 _qty) public onlyOwner{
    minBuyQtyWeed=_qty;
  }

  function setMinBuyQtyCoke(uint256 _qty) public onlyOwner{
    minBuyQtyCoke=_qty;
  }

  function setHomeDistrictRaiding(bool _enabled) public onlyOwner{
    homeDistrictRaiding = _enabled;
  }
  ...
  function setEndCooldownNowtPrice(uint256 _price) public onlyOwner{
    endCoolDownPrice = _price;
  }

  function setSaleDangerZone(uint256 _sdz) public onlyOwner{
    require(_sdz<=100);
    saleDangerZone = _sdz;
  }
  ...
  function createNamedDistrict(uint256 _index, string _name, bool _hasMarket) public onlyOwner{
    ...
    districts[_index].weedPot = 1 ether; // POA
    districts[_index].cokePot = 1 ether; // POA
  }
  ...
  function updateConsumable(uint256 _narcoId,  uint256 _index ,uint8 _newQuantity) public payable onlyTokenContract {
    narcoCore.updateConsumable(_narcoId,  _index, _newQuantity);
  }

  function updateWeedTotal(uint256 _narcoId,  uint16 _total) public payable onlyTokenContract {
    narcoCore.updateWeedTotal(_narcoId,  true , _total);
    districts[getNarcoLocation(_narcoId)].weedAmountHere += uint8(_total);
  }

  function updatCokeTotal(uint256 _narcoId,  uint16 _total) public payable onlyTokenContract {
  ...
  function distributeRevenue(uint256 _district , uint8 _splitW, uint8 _splitC) public payable onlyTokenContract {
        if(msg.value>0){
  ...
  function _distributeDopeRevenue(uint256 _grossRevenue, uint256 _district , uint8 _splitW, uint8 _splitC) internal {
          // subtract dev fees
          uint256 onePc = _grossRevenue/100;
          uint256 netRevenue = onePc*(100-devFeePercent);
          uint256 devFee = onePc*(devFeePercent);
          ...
          // distribute district revenue
          // split evenly between weed and coke pots
          districts[_district].weedPot+=(netRevenue/100)*_splitW;
          districts[_district].cokePot+=(netRevenue/100)*_splitC;
          // acrue dev fee
          currentDevFees+=devFee;
  }
  ...
  function buyItem(uint256 _narcoId, uint256 _district, uint256 _itemIndex, uint256 _quantity) public payable whenNotPaused{
    ...
    if (_itemIndex ==0 ) {
      require (_quantity>=minBuyQtyWeed);
    ...
    if (_itemIndex ==1 ) {
      require (_quantity>=minBuyQtyCoke);
  ...
  function saleBusted(uint256 _narcoId, uint256 _district , uint256 narcoWeedTotal , uint256 narcoCokeTotal , uint256[6] narcoCooldowns) internal returns (bool isBusted) {

    // travel = 0, hijack = 3
    if (now<narcoCooldowns[0] || now<narcoCooldowns[3]){
      uint256 activeCooldown = narcoCooldowns[0];
      uint256 maxCooldown = getTravelCooldownMax(_narcoId);

      if (narcoCooldowns[3]>narcoCooldowns[0]){
        activeCooldown = narcoCooldowns[3];
        maxCooldown = getHijackCooldownMax(_narcoId);
      }

      uint256 remainingCool = activeCooldown - now;
      uint256 remainingPercent = (remainingCool*100/maxCooldown);
      uint256 elapsedPercent = 100-remainingPercent;

      if (elapsedPercent<saleDangerZone){

        if (random(100) > 100-(saleDangerZone-elapsedPercent)) {
             busted(_narcoId,_district,narcoWeedTotal,narcoCokeTotal);
            return true;
        }else{
            return false;
        }
  ...
  function sellItem(uint256 _narcoId, uint256 _district, uint256 _itemIndex, uint256 _quantity) public whenNotPaused{
    ...
    require(getNarcoLocation(_narcoId) == _district && getNarcoHomeLocation(_narcoId)!=_district); // right place to sell, and not at home
    ...
    // if selling in cooldown dangerzone, there is a risk of bust by lardass

    if (saleBusted(_narcoId, _district,narcoWeedTotal ,narcoCokeTotal ,  narcoCooldowns)==true)
    {
      return;
    }
    ...
    // transfer the amount to the seller - unless contract
    if (tx.origin == msg.sender){
      msg.sender.transfer(salePrice*_quantity);
    }else{
      currentDevFees+=(salePrice*_quantity);
    }
    SaleComplete(_narcoId, salePrice*_quantity);
  }


     function setNarcoLocation(uint256 _narcoId , uint8 _district) public onlyOwner{
          narcoIndexToLocation[_narcoId] = _district;
      }

    function withdrawFees() external onlyOwner {
          if (currentDevFees<=address(this).balance){
            uint256 transferAmount = currentDevFees;
            currentDevFees = 0;
      ...
      function endCoolDownNow(uint256 _narcoId , uint256 _cooldown) public payable {
        require(narcoCore.ownerOf(_narcoId) == msg.sender); // must be owner
        require(msg.value >= endCoolDownPrice);
        narcoCore.setCooldown( _narcoId ,  _cooldown , now);
        _distributeRevenue(msg.value, getNarcoLocation(_narcoId) , 50, 50);
      }
  ...
  function travelTo(uint256 _narcoId, uint256 _exitId) public payable whenNotPaused{
    require(narcoCore.ownerOf(_narcoId) == msg.sender && msg.value >= travelPrice); // must be owner
    ...
    require(now>narcoCooldowns[0] && (narcoConsumables[0]>0 || msg.value >= airLiftPrice));

    uint8 sourceLocation = getNarcoLocation(_narcoId);
    //District storage sourceDistrict = districts[sourceLocation]; // find out source
    require(msg.value >= airLiftPrice || districts[sourceLocation].exits[_exitId] != 0); // must be a valid exit
    ...
    // do the move
    uint8 targetLocation =  uint8(_exitId);
    if (msg.value < airLiftPrice){
      targetLocation=uint8(districts[sourceLocation].exits[_exitId]);
    }
    ...
    if (msg.value < airLiftPrice){
        ...
        narcoCore.setCooldown( _narcoId ,  0 , now + getTravelCooldownMax(_narcoId));

        // update travel stat
        narcoCore.incrementStat(_narcoId, 7);
        // Travel risk
       if (calculationsContract.isBusted(narcoSkills[0]) || tx.origin != msg.sender){
  ...
  function getSkills(uint256 _narcoId) public view returns (uint16[6] narcoSkills) {
    (,,,narcoSkills,,,,,,,) = narcoCore.getNarco(_narcoId);
  }
  ...
  function hijack(uint256 _hijackerId, uint256 _victimId)  public payable whenNotPaused{
    ...
    }else
    {
      ...
      //require(victimLevel>=minRepectToRaid); // cannot raid Narcos with 0 respect

      if (homeDistrictRaiding==false){
         // victim is not in home location , or is officer lardass
        require(getNarcoLocation(_victimId)!=victimHomeLocation || _victimId==0);
      }

        // must be after travel cooldown
      if (getNarcoLocation(_victimId)==victimHomeLocation){
          require (now>victimCooldowns[0]);
      }
      ...
      // 3 = attackIndex
      // 4 = defenseIndex
      // old calc: random((hijackerSkills[3]+victimSkills[4]))+1 >victimSkills[4]
      if (calculationsContract.isRaided(narcoCore.getLevel(_hijackerId), narcoCore.getLevel(_victimId), getSkills(_hijackerId)[3] , getSkills(_victimId)[4])) {
        ...
        // successfully defended unless lardass
        if (_victimId!=0) {narcoCore.incrementStat(_victimId, 4);} // defendedSuccessfully
    ...
    //narcoCore.setCooldown( _hijackerId ,  3 , now + min(3 minutes,(455-(5*hijackerSkills[3])* 1 seconds))); // cooldown
     narcoCore.setCooldown( _hijackerId ,  3 , now + getHijackCooldownMax(_hijackerId)); // cooldown
  ...
  function getHijackCooldownMax(uint256 _hijackerId) public view returns (uint256 hijackCooldown){
    return (455-(5*getSkills(_hijackerId)[3])* 1 seconds);
  }

  function getTravelCooldownMax(uint256 _travellerId) public view returns (uint256 travelCooldown){
    return (455-(5*getSkills(_travellerId)[0])* 1 seconds);
  }
